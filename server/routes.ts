...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/send-otp", async (req, res) => {
      ...
      try {
          ...
          res.status(500).json({ error: "Gagal menyimpan OTP di Database / Kirim Email." });
      }
  });
  ...
  app.post("/api/auth/reset-password", async (req, res) => {
      ...
      try {
          ...
          if (rows.length === 0 || rows[0].code.trim() !== code.trim()) return res.status(400).json({ error: "Kode OTP Salah atau Kadaluarsa!" });
  ...
  app.get("/api/ping", async (req, res) => {
      try {
          // 🚀 OBAT PENYEMBUH DATABASE: Mengizinkan koma/desimal secara paksa saat user buka aplikasi
          try {
              await db.execute(sql`ALTER TABLE forex_assets ALTER COLUMN amount TYPE DOUBLE PRECISION;`);
              await db.execute(sql`ALTER TABLE debts ALTER COLUMN amount TYPE DOUBLE PRECISION;`);
              await db.execute(sql`ALTER TABLE investments ALTER COLUMN quantity TYPE DOUBLE PRECISION;`);
  ...
  const isAdminValid = (email: string) => {
      return ["adrienfandra14@gmail.com", "bilanotech@gmail.com"].includes(email);
  };
  ...
  app.post("/api/chat/ask", async (req, res) => {
      ...
      const systemPrompt = `Kamu adalah BILANO Intelligence, asisten konsultan keuangan tingkat elit dan mentor privat di aplikasi BILANO.
      Pembuatmu adalah Adrien Ahza Dhiafandra.
      PERATURAN SIKAP & LOGIKA KEUANGAN (MUTLAK):
      1. INGAT KONTEKS: Kamu menerima riwayat percakapan. Jika pengguna bertanya hal lanjutan, jawablah menyambung dengan topik sebelumnya.
      2. MENTOR PROAKTIF: Jadilah mentor yang peduli dan cerdas. WAJIB akhiri dengan pertanyaan penawaran bantuan.
      3. HUKUM AKUNTANSI BILANO: Menghapus/Ikhlas Piutang (Write-off) TIDAK mengurangi Kas likuid, hanya mengurangi Kekayaan Bersih. Amal dianggap pengeluaran positif.
      ...
      [DATA BULAN INI SAJA]
      - Pengeluaran Konsumtif: Rp ${pengeluaranBulanIni.toLocaleString('id-ID')}
      - Total Amal: Rp ${totalAmalBulanIni.toLocaleString('id-ID')}
      - Sisa Budget: ${sisaBudget}
      Jawab dengan Markdown rapi, berwibawa, langsung ke inti.`;
          ...
          if (txToDelete.type === 'invest_buy') newBalance += amt; 
              ...
              try {
                  ...
                  if (match) {
                      ...
                      if (existingForex) await storage.updateForexAsset(existingForex.id, Math.max(0, existingForex.amount - qty));
                  }
              } catch(e) {}
          }
      ...
      try {
          ...
          if (!isFromTransaction) {
              ...
              if (curr === 'IDR') {
                  ...
                  else currentForexAmount = Math.max(0, currentForexAmount - amount); 
  ...
  app.post("/api/debts/:id/restore", async (req, res) => {
      try {
          ...
          if (curr !== 'IDR' && hasValasTx) {
              const existingForex = await storage.getForexByCurrency(user!.id, curr);
              let currentForexAmount = existingForex ? existingForex.amount : 0;
              if (debt.type === 'piutang') currentForexAmount = Math.max(0, currentForexAmount - debt.amount); 
          ...
          if (isWriteOff) {
              ...
              if (curr === 'IDR') {
                  ...
                  if (debt.type === 'piutang') { 
                      ...
                      currentForexAmount = Math.max(0, currentForexAmount - payAmount); 
  ...
  app.patch("/api/target/penalty", async (req, res) => { 
      const user = await getUser(req); 
      try { await storage.updateTargetPenalty(user!.id, Math.round(req.body.amount)); res.json({success:true}); } catch(e) { res.status(500).send("Error"); } 
  });
  ...
  app.post("/api/target", async (req, res) => { 
      ...
      if (initialForexList && Array.isArray(initialForexList)) {
          for (const item of initialForexList) {
              if (item.amount > 0) promises.push((async () => {
  ...
  app.post("/api/investments/buy", async (req, res) => { 
      try {
          ...
          if(user!.cashBalance < total) return res.status(400).json({message:"Saldo Rupiah tidak cukup."}); 
  ...
  app.get("/api/admin/users", async (req, res) => {
      ...
      try { const allUsers = await db.select().from(users).orderBy(desc(users.createdAt)); res.json(allUsers); } catch (e) { res.status(500).json({ error: "Error" }); }
  });
  ...
  app.post("/api/help/submit", async (req, res) => {
      try {
          ...
          try { await db.execute(sql`INSERT INTO help_tickets (id, user_id, email, name, subject, message, status) VALUES (${ticketId}, ${user.id}, ${user.email}, ${name}, ${subject}, ${message}, 'Menunggu Balasan')`); } catch (dbErr) { }
          try {
              if (process.env.EMAIL_USER && process.env.EMAIL_PASS) {
                  const transporter = createTransporter();
                  await transporter.sendMail({
                      ...
                      html: `<div style="font-family: Arial, sans-serif; padding: 20px;"><h2>Tiket #${ticketId}</h2><p>${message}</p></div>`
                  });
              }
          } catch(e) { }
  ...
  app.post("/api/admin/help/reply", async (req, res) => {
      ...
      try {
          if (process.env.EMAIL_USER && process.env.EMAIL_PASS) {
              const transporter = createTransporter();
              await transporter.sendMail({ from: `"Tim Bantuan BILANO" <${process.env.EMAIL_USER}>`, to: userEmail, subject: `Re: [${ticketId}] ${subject}`, html: `<p>${replyMessage}</p>` });
          }
  ...
  app.post("/api/admin/silent-correction", async (req, res) => {
      try {
          ...
          await storage.updateUserBalance(user!.id, newBalance);
          res.json({ success: true, message: "Koreksi berhasil." });
      } catch(e:any) { res.status(500).json({ error: e.message }); }
  });
  ...
  app.post("/api/vision/scan", async (req, res) => {
      try {
          ...
          const systemPrompt = `Cari "TOTAL" dari gambar struk. Output JSON murni: {"totalAmount": 150000, "currency": "IDR", "category": "Makan/Minum", "description": "Makan"}`;
          ...
          let parsedResult;
          try { parsedResult = JSON.parse(resultText); } catch (e) { parsedResult = JSON.parse(resultText.replace(/```json/g, '').replace(/```/g, '').trim()); }
