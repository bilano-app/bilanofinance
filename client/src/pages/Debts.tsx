...
import { 
    ...
    CheckCircle2, Trash2, Plus, HandCoins, AlertCircle, X
} from "lucide-react";
...
export default function Debts() {
  ...
  const handleAdd = async () => {
      ...
      try {
          const res = await fetch("/api/debts", {
              ...
              body: JSON.stringify({ type: activeTab, name, amount: parseFloat(amount), dueDate, description: desc })
          });
          if(res.ok) {
              toast({ title: "Tersimpan", description: "Catatan berhasil ditambahkan." });
              setIsFormOpen(false); setName(""); setAmount(""); setDueDate(""); setDesc("");
              fetchData();
          }
      } catch (e) { toast({ title: "Error", variant: "destructive" }); }
  };

  const handleMarkPaid = async (id: number, type: string, name: string) => {
      if(!confirm(`Tandai Lunas untuk ${name}? Saldo akan otomatis disesuaikan.`)) return;
      try {
  ...
  const totalAmount = filteredItems.filter(i => !i.isPaid).reduce((acc, curr) => acc + curr.amount, 0);
      ...
      <div className="space-y-6 pt-4 pb-24 px-1">
        ...
        <div className={`p-6 rounded-[32px] text-white shadow-xl relative overflow-hidden transition-colors duration-500 ${activeTab === 'piutang' ? 'bg-gradient-to-br from-emerald-500 to-teal-700' : 'bg-gradient-to-br from-rose-500 to-pink-700'}`}>
            <div className="relative z-10">
                ...
                <h2 className="text-4xl font-extrabold tracking-tight">{formatRp(totalAmount)}</h2>
            </div>
        ...
        {!isFormOpen ? (
            ...
            <div className="bg-white p-6 rounded-[32px] shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-slate-100 animate-in zoom-in-95">
                ...
                <div className="space-y-4">
                    ...
                    <Input type="number" placeholder="Nominal (Rp)" value={amount} onChange={e => setAmount(e.target.value)} className="h-14 rounded-[20px] bg-slate-50 border-transparent font-bold text-lg"/>
                ...
                filteredItems.map(item => (
                    <div key={item.id} className={`bg-white p-5 rounded-[24px] border shadow-[0_4px_20px_rgb(0,0,0,0.03)] flex justify-between items-center transition-all ${item.isPaid ? 'opacity-60 border-slate-100' : (activeTab === 'piutang' ? 'border-emerald-50' : 'border-rose-50')}`}>
                        <div className="flex-1 mr-4">
                            ...
                            <p className={`font-extrabold text-lg mt-1 ${item.isPaid ? 'text-slate-400 line-through' : 'text-slate-800'}`}>
                                {formatRp(item.amount)}
                            </p>
                            <div className="text-[11px] text-slate-500 font-medium flex items-center gap-1 mt-1.5">
                        ...
                        <div className="flex flex-col gap-2">
                            {!item.isPaid ? (
                                <button onClick={() => handleMarkPaid(item.id, item.type, item.name)} className={`p-3 rounded-[16px] text-white shadow-md active:scale-95 transition-transform flex flex-col items-center justify-center ${activeTab === 'hutang' ? 'bg-rose-500 hover:bg-rose-600' : 'bg-emerald-500 hover:bg-emerald-600'}`}>
