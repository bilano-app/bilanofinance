  ...
  const generatePDF = (targetMonth?: number, targetYear?: number, isYearly: boolean = false) => {
    ...
    setTimeout(() => {
        try {
            ...
            // =================================================================================
            // 🚀 ALGORITMA KALIBRASI GAIB (Mengobati Kas Minus Akibat Edit Manual)
            // =================================================================================
            let totalNetTx = 0;
            allTxs.forEach((t:any) => {
                ...
                if (!isNonCash) {
                    if (['income', 'debt_borrow', 'debt_receive', 'invest_sell', 'forex_sell', 'piutang_record'].includes(t.type)) totalNetTx += amt;
                    else if (['expense', 'debt_lend', 'debt_pay', 'invest_buy', 'forex_buy', 'hutang_record'].includes(t.type)) totalNetTx -= amt;
                }
            });
            // Menemukan Modal Awal yang tidak tercatat (Live Cash vs Transaksi)
            const baseCash = Number(user.cashBalance || 0) - totalNetTx;
            ...
            const getSnapshotAt = (targetDate: Date) => {
                ...
                // Memastikan kas tunai tidak menjebol kalkulasi Net Worth jika ada data anomali
            ...
            // Loop ke belakang untuk membuat titik grafik yang Independent tiap bulannya!
            ...
            // TIDAK ADA LAGI KODE CHEAT! SEMUANYA MURNI DARI DATABASE!
