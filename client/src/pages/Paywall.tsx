  ...
  const handleLanjutBayar = async () => {
      ...
      try {
          ...
          if (res.ok && data.redirectUrl) {
              // 🚀 Status PRO hanya akan aktif murni dari database (menunggu konfirmasi webhook Bank)
                        ...
                        <div onClick={() => setSelectedPlan('yearly')} className={`relative p-5 rounded-[20px] border-2 cursor-pointer transition-all overflow-hidden ${selectedPlan === 'yearly' ? 'border-amber-400 bg-gradient-to-br from-slate-900 to-indigo-950 shadow-xl' : 'border-slate-200 bg-white'}`}>
                            ...
                            {selectedPlan === 'yearly' && (
                                <button 
                                    ...
                                    className="relative z-10 mt-4 w-full flex items-center justify-between bg-white/10 hover:bg-white/20 border border-white/20 p-3 rounded-xl transition-all text-left group"
                                >
